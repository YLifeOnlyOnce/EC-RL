...
class EdgeEnv:
    ...
    def initState(self):
        ...
        # 计算每个任务可卸载的边缘节点位置
        self.cov_matrix = np.ones((self.userNum, self.edgeNum), dtype=np.int16)
        self.edgeComputingAllocation_matrix = np.zeros((self.userNum, self.edgeNum), dtype=np.int16)
        self.offload_percent_matrix = np.zeros((self.userNum, self.edgeNum + 1), dtype=np.float)
        ...
        # 构造 state
        self.state = np.concatenate([self.offload_percent_matrix.flatten(), self.edgeComputingAllocation_matrix.flatten()])
        ...
        if edgeindex != 0:
            ...
            if act == 0:
                ...
                self.offload_percent_matrix[userpart, edgeindex] = min(
                    self.offload_percent_matrix[userpart, edgeindex] + 0.005, 1)
                self.offload_percent_matrix[userpart, 0] = max(self.offload_percent_matrix[userpart, 0] - 0.005, 0)
            elif act == 2:
                print('act2')
                self.offload_percent_matrix[userpart, edgeindex] = max(
                    self.offload_percent_matrix[userpart, edgeindex] - 0.005, 0)
                self.offload_percent_matrix[userpart, 0] = min(self.offload_percent_matrix[userpart, 0] + 0.005, 1)
        print(self.offload_percent_matrix)

    def changeComputingAllocation(self):
        self.edgeComputingAllocation_matrix = self.edgeComputingAllocation_matrix + np.random.randint(7000,13000, (self.userNum, self.edgeNum))
        print(self.edgeComputingAllocation_matrix)
        return
    ...
    def computing_time_task(self, i):
        # 时间计算 1。单个任务时间，一个函数
        # 2。 任务总体完成时间，一个函数
        t_local = (self.offload_percent_matrix[i, 0] * self.users[i].task.required_computing_resource) / self.users[i].computing_resource
        print("本地计算时间{}".format(t_local))

        # 卸载的子任务
        A = lambda x:self.fuc(x)
        tasks_required_computing_resource = [A(x) for x in self.users]
        print("任务的总需计算量:{}".format(tasks_required_computing_resource))

        offload_computing_resource_matrix = self.offload_percent_matrix[i, 1:] * tasks_required_computing_resource
        print("卸载的计算资源:{}".format( offload_computing_resource_matrix )  )

        # 远程计算时间计算
        t_edge_total = np.max(offload_computing_resource_matrix[i] / self.edgeComputingAllocation_matrix[i])
        print(t_edge_total)

        t_total_i = max(t_local, t_edge_total)
        return t_total_i
    ...
    def reward(self, action):
        ...
        r = 100 / self.computing_time_task(0)
        return r
